...
# Internal helpers used by saturation-aware stable-lambda selection.
CECextractFitParams <- function(obj) {
  candidates <- list(
...
CECboundSaturationTable <- function(best_partitions_obj) {
  best_list <- best_partitions_obj$best
  lambda <- if (!is.null(best_partitions_obj$summary) &&
      "lambda" %in% names(best_partitions_obj$summary)) {
    best_partitions_obj$summary$lambda
  } else {
    vapply(best_list, function(x) {
      if (is.null(x) || is.null(x$lambda)) {
        NA_real_
      } else {
        as.numeric(x$lambda)[1]
      }
    }, numeric(1))
  }
  ...
  for (i in seq_along(best_list)) {
    ...
    params <- CECextractFitParams(obj)
    if (is.null(params)) next

    nu <- suppressWarnings(as.numeric(params$nu))
    if (length(nu) == 0L || all(!is.finite(nu))) next
...
#'   the selected stable region is further restricted to lambda values whose
#'   saturation share stays below `sat_threshold`.
#' @param ratio_threshold,stabB_threshold,sat_threshold Thresholds used by the
#'   selected stability rule and optional saturation filter.
...
#' @param best_partitions_obj Optional object returned by
#'   [CECextractBestPartitions()]. When supplied, the function derives the
#'   saturation share `sat`, defined as the proportion of the sample living in
#'   clusters whose fitted density reaches the upper bound `C`.
#' @param ratio_col,stabB_col,sat_col Column names searched in
#'   `lambda_diag$summary`.
...
#'   a compact summary data frame. If saturation information is available, the
#'   summary also reports whether a lambda is excluded specifically because of
#'   bound saturation.
#' @export
CECidentifyStableLambdas <- function(
  ...
  if (!(sat_col %in% names(df))) {
    if (!is.null(best_partitions_obj)) {
      sat_df <- CECboundSaturationTable(best_partitions_obj)
      idx <- match(df$lambda, sat_df$lambda)
      df[[sat_col]] <- sat_df$sat[idx]
      if (!("n_sat_clusters" %in% names(df))) {
        df$n_sat_clusters <- sat_df$n_sat_clusters[idx]
      }
      if (!("any_bound_saturation" %in% names(df))) {
        df$any_bound_saturation <- sat_df$any_bound_saturation[idx]
      }
    } else {
      df[[sat_col]] <- NA_real_
      if (!("n_sat_clusters" %in% names(df))) df$n_sat_clusters <- NA_integer_
      if (!("any_bound_saturation" %in% names(df))) df$any_bound_saturation <- NA
    }
  }
  ...
  cond_stability <- switch(
    rule,
    ratio = cond_ratio,
    stabB = cond_stabB,
    both = cond_ratio & cond_stabB
  )
  cond_stability[is.na(cond_stability)] <- FALSE
  cond_sat <- is.na(df[[sat_col]]) | (df[[sat_col]] < sat_threshold)
  cond_sat[is.na(cond_sat)] <- TRUE

  stable_info_stability <- CECstableRunMask(cond_stability, min_consecutive = min_consecutive)
  stable_stability <- stable_info_stability$mask
  stable_stability[is.na(stable_stability)] <- FALSE

  cond <- cond_stability & cond_sat
  ...
  stability_runs <- stable_info_stability$runs
  if (nrow(stability_runs) > 0) {
    stability_runs$lambda_start <- df$lambda[stability_runs$start]
    stability_runs$lambda_end <- df$lambda[stability_runs$end]
  } else {
    stability_runs$lambda_start <- numeric(0)
    stability_runs$lambda_end <- numeric(0)
  }
  ...
  list(
    ...
    unstable_stability = !stable_stability,
    exclude_saturation = exclude_saturation,
    raw_condition = cond,
    raw_condition_stability = cond_stability,
    raw_condition_saturation = cond_sat,
    runs = runs,
    ...
    summary = data.frame(
      ...
      stabB = df[[stabB_name]],
      sat = df[[sat_col]],
      n_sat_clusters = df$n_sat_clusters,
      any_bound_saturation = df$any_bound_saturation,
      keep_stability = stable_stability,
      keep_saturation = cond_sat,
      exclude_saturation = exclude_saturation,
      condition_stability = cond_stability,
      condition_sat = cond_sat,
      condition = cond,
      stable = stable_info$mask
    )
  )
...
CECidentifyLambdaMin <- function(
  ...
  out <- CECidentifyStableLambdas(
    ...
    use_smoothed = use_smoothed,
    min_consecutive = min_consecutive,
    best_partitions_obj = best_partitions_obj,
    ratio_col = ratio_col,
    stabB_col = stabB_col,
    sat_col = sat_col
  )
...
CECextractSummaryMask <- function(lambda, lambda_stable_obj, summary_col, fallback_name = NULL) {
  if (is.null(lambda_stable_obj)) return(NULL)

  if (!is.null(lambda_stable_obj$summary) && "lambda" %in% names(lambda_stable_obj$summary)) {
    summary_df <- lambda_stable_obj$summary
    if (summary_col %in% names(summary_df)) {
      idx <- match(lambda, summary_df$lambda)
      out <- rep(NA, length(lambda))
      ok <- !is.na(idx)
      out[ok] <- as.logical(summary_df[[summary_col]][idx[ok]])
      return(out)
    }
  }

  if (!is.null(fallback_name) &&
      !is.null(lambda_stable_obj[[fallback_name]]) &&
      length(lambda_stable_obj[[fallback_name]]) == length(lambda)) {
    return(as.logical(lambda_stable_obj[[fallback_name]]))
  }

  NULL
}

CECextractStabilityMask <- function(lambda, lambda_stable_obj) {
  mask <- CECextractSummaryMask(
    lambda = lambda,
    lambda_stable_obj = lambda_stable_obj,
    summary_col = "keep_stability",
    fallback_name = "stable_stability"
  )
  if (!is.null(mask)) return(mask)
  CECextractStableMask(lambda, lambda_stable_obj)
}

CECextractSaturationMask <- function(lambda, lambda_stable_obj) {
  mask <- CECextractSummaryMask(
    lambda = lambda,
    lambda_stable_obj = lambda_stable_obj,
    summary_col = "exclude_saturation",
    fallback_name = "exclude_saturation"
  )
...
CECshadeLambdaBands <- function(
  ...
  mark = c("shade", "hatch", "none"),
  fill_col = grDevices::adjustcolor("white", alpha.f = 0.55),
  border_col = grDevices::adjustcolor("grey10", alpha.f = 0.75),
  border_lwd = 0.8,
  density = 12,
  angle = 45
) {
  ...
  unstable_runs <- split(
    unstable_idx,
    cumsum(c(TRUE, diff(unstable_idx) != 1L))
  )

  for (run_idx in unstable_runs) {
    lower <- min(bands$lower[run_idx])
    upper <- max(bands$upper[run_idx])
    if (axis == "x") {
      xleft <- lower
      xright <- upper
      ybottom <- usr[3]
    ...
    if (mark == "shade") {
      ...
      rect(
        xleft, ybottom, xright, ytop,
        col = grDevices::adjustcolor("white", alpha.f = 0),
        border = border_col,
        lwd = border_lwd
      )
    }
...
#'   to shade stable and unstable regions. When the stable-object carries
#'   saturation diagnostics, lambdas excluded by saturation can be highlighted
#'   separately.
#' @param unstable_col Colour used to shade unstable lambda bands.
#' @param saturation_mark Visual mark used for lambdas excluded by the
#'   saturation filter only. `"hatch"` uses reverse diagonal hatching, `"shade"`
#'   uses a semi-transparent overlay, and `"none"` disables the extra layer.
#' @param saturation_col,saturation_fill_col,saturation_border_col,
#'   saturation_border_lwd,saturation_density,saturation_angle Graphical
#'   parameters used when drawing saturation-only exclusions.
...
plotCECdiagnoseLambdaGrid <- function(
  ...
  saturation_mark = c("hatch", "shade", "none"),
  saturation_col = grDevices::adjustcolor("grey15", alpha.f = 0.6),
  saturation_fill_col = grDevices::adjustcolor("white", alpha.f = 0.15),
  saturation_border_col = grDevices::adjustcolor("grey10", alpha.f = 0.75),
  saturation_border_lwd = 0.8,
  saturation_density = 12,
  saturation_angle = -45
) {
  saturation_mark <- match.arg(saturation_mark)
  df <- lambda_diag$summary
  ...
  stable_mask <- CECextractStabilityMask(df$lambda, lambda_stable_obj)
  saturation_mask <- CECextractSaturationMask(df$lambda, lambda_stable_obj)
  ...
  plot_metric <- function(y, ylab, main, ylim = NULL, h = NULL) {
    ...
    CECshadeLambdaBands(df$lambda, stable_mask, axis = "x", col = unstable_col, mark = "shade")
    if (!is.null(saturation_mask) && saturation_mark != "none") {
      CECshadeLambdaBands(
        df$lambda,
        saturation_mask,
        axis = "x",
        col = saturation_col,
        mark = saturation_mark,
        fill_col = saturation_fill_col,
        border_col = saturation_border_col,
        border_lwd = saturation_border_lwd,
        density = saturation_density,
        angle = saturation_angle
      )
    }
...
plotCECPartitionEvolution1D <- function(
  ...
  stable_mask <- CECextractStabilityMask(lambda_all, lambda_stable_obj)
  saturation_mask <- CECextractSaturationMask(lambda_all, lambda_stable_obj)
  ...
  if (is.null(lambda_subset)) {
    ...
    CECshadeLambdaBands(lambda_all, stable_mask, axis = "y", col = unstable_col, mark = "shade")
    if (!is.null(saturation_mask) && saturation_mark != "none") {
      CECshadeLambdaBands(
        lambda_all,
        saturation_mask,
